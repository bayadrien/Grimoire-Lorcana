...
export default function ScanPage() {
  ...
  function crop(video: HTMLVideoElement, top: number, height: number) {
    const canvas = document.createElement("canvas");
    const context = canvas.getContext("2d");
    canvas.width = Math.min(1400, video.videoWidth);
    canvas.height = Math.max(1, Math.round(canvas.width * ((video.videoHeight * height) / video.videoWidth)));
    context?.save();
    if (context) context.filter = "grayscale(1) contrast(2)";
    context?.drawImage(video, 0, video.videoHeight * top, video.videoWidth, video.videoHeight * height, 0, 0, canvas.width, canvas.height);
    context?.restore();
    return canvas;
  }
  ...
  async function analyze() {
    ...
    try {
      ...
      const captures = [crop(video, .76, .22), crop(video, .76, .22), crop(video, .76, .22)];
      const titleCrop = crop(video, .39, .20);
      ...
      setStatus("Lecture du nom…");
      const title = (await ocr.recognize(titleCrop)).data.text.replace(/\s+/g, " ").trim();
      const number = mostFrequent(readings);
      if (!number && !title) { setStatus("Le texte n’a pas été lu. Cadre le bas de la carte sans reflet, ou utilise la recherche manuelle."); return; }
      setManual(number || title);
      await search(readings.join(" "), title);
    } catch (error) {
