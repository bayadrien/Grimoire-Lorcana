...
export default function EchangePage() {
  ...
  // reload on filters (debounce mini)
  useEffect(() => {
    ...
    <main className="shell">
      ...
      <div className="topbar" style={{ marginTop: 12, justifyContent: "space-between" }}>
        ...
        <div style={{ opacity: 0.85 }}>
          {loading ? "⏳" : data ? (
            <>
              Adrien→Angèle: <b>{data.summary.adrienToAngeleCount}</b> cartes (copies:{" "}
          ...
          {a2g.map((r) => (
            <article key={r.card.id} className="card">
              <div className="cardMedia">
                <img
                  src={r.card.imageUrl || PLACEHOLDER}
                  alt={r.card.name}
                  loading="lazy"
                />
                ...
                <div className="overlay">
                  ...
                  <div className="ovMeta">
                    {r.card.setName}{r.card.setCode ? ` • Chapitre ${r.card.setCode}` : ""}<br />
                    {tInk(r.card.ink)} • {tRarity(r.card.rarity)} • Coût {r.card.cost ?? "—"}<br />
