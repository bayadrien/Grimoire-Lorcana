...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [collection, openings, deckCards, trades, related] = await Promise.all([
    ...
    return NextResponse.json({ card, collection: { normal, foil, total: normal + foil, english: collection.some((row) => row.isEnglish), unitValue: Number(Math.max(normalPrice, foilPrice).toFixed(2)), estimatedValue: Number((normal * normalPrice + foil * foilPrice).toFixed(2)) }, openings: { count: openings.length, first: openings[0] || null, latest: openings.slice(-5).reverse() }, decks: deckCards.map((entry) => ({ ...entry.deck, quantity: entry.quantity })), trades, related });
  } catch (error) {
