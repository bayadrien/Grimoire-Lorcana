...
type Card = { id:string; setCode?:string|null; setName?:string|null; collection_number?:string|null; ink?:string|null; rarity?:string|null; usd?:number|null; usd_foil?:number|null; name_fr?:string|null; name?:string|null; imageUrl?:string|null };
...
const referenceOf = (c:Card) => "Chapitre " + (c.setCode || "—") + " · #" + (c.collection_number || "—");
...
export default function StatsPage() {
  ...
  return <><AppHeader/><main className="stats-page"><div className="wrap">
    ...
    <section className="grid-bottom"><article className="panel"><Header title="Les pépites du grimoire" label="VALEUR" badge="Top 5"/>{stats.top.length? <div className="top">{stats.top.map((x,i)=><div key={x.card.id}><span>0{i+1}</span><span className="card-visual">{x.card.imageUrl?<img src={x.card.imageUrl} alt={nameOf(x.card)}/>:<i>🎴</i>}</span><p><b>{nameOf(x.card)}</b><small>{tRarity(x.card.rarity)} · {total(x.q)} exemplaire{total(x.q)>1?"s":""}{x.q.foil?" · "+x.q.foil+" foil":""} · {referenceOf(x.card)}</small></p><strong>{amount(x.value)}</strong></div>)}</div>:<p className="empty">Aucune carte dans cette collection.</p>}</article>
...
,".top .card-visual{display:block;position:relative;z-index:1;width:36px;height:50px;transition:transform .2s ease}.top .card-visual:hover{transform:scale(3.25);z-index:20;filter:drop-shadow(0 12px 16px rgba(33,27,48,.35))}.top .card-visual img,.top .card-visual i{display:block;width:36px;height:50px;transition:box-shadow .2s ease}.top>div:hover{position:relative;z-index:3}.top p small{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}@media(max-width:480px){.top .card-visual{width:32px;height:45px}.top .card-visual img,.top .card-visual i{width:32px;height:45px}.top .card-visual:hover{transform:scale(2.5);transform-origin:left center}}"
].join("");
